...
export def everything []: string -> path, nothing -> path {
  ...
  if ($query | is-not-empty) {
    fzf --bind $"start:reload:($esTemplate)" --bind $"change:reload-sync\(sleep 100ms; ($esTemplate)\)" --query $query --header="Search - Everything"
    return
  }
  ...
  null | fzf --bind $"change:reload-sync\(sleep 100ms; ($esTemplate)\)" --phony --query "" --header="Search - Everything"
}
