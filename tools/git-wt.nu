...
# 2. Move the generated code to this directory
#
# For windows, it moves it to the incorrect path. I.e:
#   ~/.config/nushell/vendor/autoload/
# Instead of:
#   C:\Users\<User>\AppData\Roaming\nushell\vendor\autoload
# Can check the vendor autoload directories with below command:
...
export def "nu-complete git-wt" [context: string] {
    let worktrunk_bin = if ($env.WORKTRUNK_BIN? | is-not-empty) {
        $env.WORKTRUNK_BIN
    } else {
        let external = (which -a git-wt | where type == "external")
...
# Override git-wt command with file-based directive passing.
...
#   (`git-wt list --format json | from json`). No directive processing — `list`
...
export def --env --wrapped git-wt [...args: string@"nu-complete git-wt"] {
    let worktrunk_bin = if ($env.WORKTRUNK_BIN? | is-not-empty) {
        $env.WORKTRUNK_BIN
    } else {
        ...
        if ($external | is-empty) {
            error make {
                msg: "git-wt binary not found in PATH. Install with 'cargo install --path .' or set $env.WORKTRUNK_BIN"
            }
        }
        ($external | get 0.path)
    }
